...
public class AuthorizationGroupHeader {
    ...
    public static AuthorizationGroupHeader fromUserAndPassword(ServerZkAuthOperations zkAuthOperations, String user, String password) throws InvalidAuthorizationHeaderException {
        try {
            GroupPublicParams groupPublicParams = new GroupPublicParams(hex2Bytes(user));
            AuthCredentialPresentation authCredentialPresentation = new AuthCredentialPresentation(hex2Bytes(password));
            zkAuthOperations.verifyAuthCredentialPresentation(groupPublicParams, authCredentialPresentation);
            GroupEntity groupEntity = new GroupEntity();
            groupEntity.setGroupPublicParams(groupPublicParams);
            groupEntity.setAuthCredentialPresentation(authCredentialPresentation);
    ...
    public static void main(String[] args) {
        ...
        byte[] userBytes = hex2Bytes(password);
        System.out.println(userBytes.length);
//        byte[] secretBytes = Base64.decode((serverSecret + "==").getBytes());
//        ServerSecretParams zkSecretParams = new ServerSecretParams(secretBytes);
