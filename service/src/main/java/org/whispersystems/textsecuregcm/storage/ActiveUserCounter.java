...
public class ActiveUserCounter extends AccountDatabaseCrawlerListener {
	...
	protected void onCrawlChunk(Optional<UUID> fromNumber, List<Account> chunkAccounts) {
		...
		long agoMs[] = { TimeUnit.DAYS.toMillis(nowDays - 1), TimeUnit.DAYS.toMillis(nowDays - 7),
				TimeUnit.DAYS.toMillis(nowDays - 30), TimeUnit.DAYS.toMillis(nowDays - 90),
	...
	private void incrementTallies(UUID fromUuid, Map<String, long[]> platformIncrements,
			Map<String, long[]> countryIncrements) {
		try (Jedis jedis = jedisPool.getWriteResource()) {
